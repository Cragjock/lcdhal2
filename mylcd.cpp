...
lcddisplay::lcddisplay()
{
	...
	myI2C= unique_ptr<I2CBus>(new I2CBus(1,0x20));  //myI2C=new I2CBus(1,0x20);
...
{
	//cout << "lcddisplay In sLCD constructor" << endl;
	myI2C= unique_ptr<I2CBus>(new I2CBus(1,0x20));  //myI2C=new I2CBus(1,0x20);
...
{
    int res = myI2C->device_write(GPIO, 0x00);
}
/******************************/
void lcddisplay::lcd_send_command(uint8_t command)
{
    #ifdef BITSET
    //lcd_send_byteBITSET(command);
...
void lcddisplay::lcd_send_byteBITSET(RS cmd, uint8_t b)
{
    ...
    /// align to HW output BKL DB7/3 DB6/2 DB5// DB4/0 E RS NC

        bitset<8> UPPERbits(((b & 0xf0)>>1));
        bitset<8> LOWERbits(((b & 0x0f)<<3));
    ...
    ///send high nibble (0bXXXX0000), BKL DB4/0 DB5/1 DB6/2
        //int res = myI2C->device_write(wtf1);
        //this_thread::sleep_for(chrono::microseconds(1));
        int res = myI2C->device_write(GPIO, wtf);
        this_thread::sleep_for(chrono::nanoseconds(100)); ///per spec pg 52/58 E pulse width s/b 230ns min
        res = myI2C->device_write(GPIO, wtf1);
    ...
    /// If in 8bit, done & exit, no need for sending lower nibble
        ...
        //res = myI2C->device_write(wtf1);
        this_thread::sleep_for(chrono::microseconds(1));
        res = myI2C->device_write(GPIO, wtf);
        this_thread::sleep_for(chrono::nanoseconds(100));
        res = myI2C->device_write(GPIO, wtf1);
        this_thread::sleep_for(chrono::nanoseconds(100));
        return;
